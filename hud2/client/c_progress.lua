...
local controls = {
    ...
    disableMovement = { 30, 31, 36, 21, 75, 0, 36, 18, 22, 55, 76, 102, 143, 177, 179, 194, 202, 203, 216, 255, 298, 321, 328, 353 },
        ...
        while isDoingAction do
            ...
            if Action.animation then
                if Action.animation.task then
                    local task = Action.animation.task
                    if not IsPedUsingScenario(ped, task) then
                        TaskStartScenarioInPlace(ped, task, 0, true)
                    end
                else
                    local anim = Action.animation
                    if anim.animDict and anim.anim and DoesEntityExist(ped) and not IsEntityDead(ped) and not RTCore.Functions.GetPlayerData().metadata['isdead'] then
                        loadAnimDict(anim.animDict)
                        if not IsEntityPlayingAnim(ped, anim.animDict, anim.anim, 3) then
    ...
    if (not isPlayerDead or action.useWhileDead) and not isDoingAction then
        ...
        SendNUIMessage(json.encode({
            ...
            data = {
                ...
                label = action.label,
                position = Utils.getMinimapPosition().pixel.top_y,
                icon = action.icon or nil, -- "fa-solid fa-pot-food"
            }
        }))
        StartActions()
        CreateThread(function()
...
RegisterNUICallback('NUI:update', function(data, cb)
    if data.type == 'FinishAction' then
        ActionCleanup()
    end
    cb('ok')
end)
...
--     exports['Rc2-Hud']:Progress({
--         name = "random_task",
--         duration = 5000,
--         label = "جاري اكل الطعام ..",
